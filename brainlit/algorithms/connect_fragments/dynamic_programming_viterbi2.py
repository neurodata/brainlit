...
class most_probable_neuron_path:
    def __init__(
        ...
        intensities = np.array([np.unique(image)]).T
        scores = kde.score_samples(intensities)

        for int, score in tqdm(zip(intensities, scores), desc="setting up emission distribution..."):
            image_tiered[image == int] = -score

        # for value in tqdm(np.unique(image), desc="setting up emission distribution..."):
        #     score = kde.score_samples(np.array([[value]]))
        #     image_tiered[image == value] = -score
