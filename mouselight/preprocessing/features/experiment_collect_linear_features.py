import numpy as np
from ..src import octree, image_getters, image_process
...
swc_dir_path = Path('/cis/net/io50/local/jacs/data/jacsstorage/samples/2018-08-01/swcs/17_9_19_consensus/2018-08-01-consensus-swcs/')
...
for j,file in enumerate(files):
    ...
    for i in range(0,num_pts,1):
        ...
        #unprocessed
        img0,start = combine_swc_img.points2img(tree,df.iloc[i:i+1].reset_index(),pad=[31,31,31])
       
        #processed
        img0 = preprocess.center(img0)
        #img0 = ndi.gaussian_filter(img0,sigma=[3,3,0.9])
        points = combine_swc_img.points2voxel(tree,df.iloc[i:i+1].reset_index(),start)
        voxels = points[['xvox','yvox','zvox']].values 
        center = voxels[0]

        training = training.append({'File No.':j,'Point':i, 'Label':0},ignore_index=True)
        features0 = np.zeros((1,num_kernels))
        training = training.append({'File No.':j,'Point':i, 'Label':1},ignore_index=True)
        ...
        for k,kernel in enumerate(kernels):
            ...
            features0[0,k] = np.dot(img0[start0[0]:end0[0],start0[1]:end0[1],start0[2]:end0[2]].flatten(),kernel.flatten())
            features1[0,k] = np.dot(img0[start1[0]:end1[0],start1[1]:end1[1],start1[2]:end1[2]].flatten(),kernel.flatten())
        ...
        if training_features is None:
            training_features = np.concatenate((features0,features1),
                                axis=0)
        else:
            training_features = np.concatenate((training_features,features0,features1),
                                axis=0)
