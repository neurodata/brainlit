...
df_iter = pd.read_csv('../large_files/neighborhood_01_24_555.csv',header=None,
    index_col=0)#, chunksize=100000, iterator=True)
...
for iter_num, df in enumerate(df_iter,1):
    ...
    for i, (train_index, test_index) in enumerate(kf.split(X)):
        ...
        if stats:
            ...
            viz = plot_roc_curve(logr, X_test, y_test,
...
def examine_failures(y_true,y_guess,locs):
    ...
    for wrong_idx in wrong_idxs:
        ...
        img0,start = combine_swc_img.points2img(tree,df.iloc[pt:pt+1].reset_index(),pad=[31,31,31])
        img0 = preprocess.center(img0)

        if true_class == 1:
            print(1)
            points = combine_swc_img.points2voxel(tree,df.iloc[pt:pt+1].reset_index(),start)
        ...
        fname = output_fail + str(file) + '_' + str(pt) + '_' + append + '_' + str(true_class) + '.png'
        plt.savefig(fname)
        plt.close()
