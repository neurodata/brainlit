...
def gabor_kernel_3d(frequency, theta=0, phi=0, sigma_x=None, sigma_y=None, sigma_z=None, n_stds=3, offset=0):
    ...
    x0 = np.ceil(max(np.abs(n_stds * sigma_x * np.cos(theta) * np.cos(phi)),
                np.abs(n_stds * sigma_y * np.sin(phi)),
                np.abs(n_stds * sigma_z * np.sin(theta) * np.cos(phi)),1))
    y0 = np.ceil(max(np.abs(n_stds * sigma_x * np.cos(theta) * np.sin(phi)),
                np.abs(n_stds * sigma_y * np.cos(phi)),
                np.abs(n_stds * sigma_z * np.sin(theta) * np.sin(phi)),1))
    z0 = np.ceil(max(np.abs(n_stds * sigma_x * np.sin(theta)),
                np.abs(n_stds * sigma_z * np.cos(theta)),1))

    x,y,z = np.mgrid[-x0:x0+1,-y0:y0+1,-z0:z0+1]

    #the rot coordinates come from the inverse rotations by theta and phi
    rotx = x*np.cos(theta)*np.cos(phi) + y*np.cos(theta)*np.sin(phi) - z*np.sin(theta)
    roty = -x*np.sin(phi) + y*np.cos(phi)
    rotz = x*np.sin(theta)*np.cos(phi) + y*np.sin(theta)*np.cos(phi) + z*np.cos(theta)

    g = np.zeros(x.shape, dtype=np.complex)
    g[:] = np.exp(-0.5 * (rotx ** 2 / sigma_x**2 + roty ** 2 / sigma_y**2 + rotz ** 2 / sigma_z**2))
...
def getKernels(neighborhood=None):
    ...
    for p,params in enumerate(product(freqs, sigs, offsets, angles)):
        ...
        filter = gabor_kernel_nd(sigma=sig,
                    phi = angle,
                    frequency=freq,
                    offset=offset)
        kernels.append(np.real(filter))
        name = str(p+4) + ' gabor_' + str(freq) + '_' + str(sig) + '_%1.2f'%offset + '_' + str(angle)
        names.append(name)
    ...
    for p,params in enumerate(product(freqs, sigs, offsets, angles)):
        ...
        filter = gabor_kernel_nd(sigma=sig,
                    ...
                    offset=offset)
        kernels.append(np.real(filter))
        name = str(p+28) + ' gabor_' + str(freq) + '_' + str(sig) + '_%1.2f'%offset + '_' + str(angle)
        names.append(name)
