...
data_bin = Path('/Users/thomasathey/Documents/mimlab/mouselight/brainlit_parent/brainlit/experiments/BrainLine/data/ilastik-timing/')
...
for radius in tqdm([2**i for i in range(8,11)], desc='different file sizes...'):
    ...
    for num_splits in tqdm([2**i for i in range(1,3)], desc="different chunkings...", leave=False):
        num_files = num_splits ** 3

        split_ras = [data]
        for axis in range(1,4):
            split_ras = [sub_ra for ra in split_ras for sub_ra in np.split(ra, num_splits, axis=axis)]
        ...
        command = [f'{ilastik_path}', '--headless', f'--project={project_path}'] + fnames
        ...
        t0 = time.time()
        for fname in fnames:
            command = [f'{ilastik_path}', '--headless', f'--project={project_path}', fname]
            subprocess.run(command,
                    stdout=subprocess.PIPE,
                    stderr=subprocess.PIPE
            )
...
data = {"Command": commands, "Number of Files": files, "Overall Image Size": image_sizes, "Time": times}
