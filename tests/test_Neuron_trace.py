...
def test_get_df_voxel():
    ...
    #test if output is correct shape
    correct_shape = (1650, 7)
    df_voxel_swc = test_swc.get_df_voxel(spacing=np.asarray([1,2,3]),origin=np.asarray([2,2,2]))
    assert df_voxel_swc.shape == correct_shape
    correct_shape = (1650, 7)
    df_voxel_s3 = test_s3.get_df_voxel(spacing=np.asarray([1,2,3]),origin=np.asarray([2,2,2]))
    ...
    #test if output is dataframe
    assert isinstance(test_swc.get_df_voxel(spacing=np.asarray([0,1,2]),origin=np.asarray([0,1,2])),pd.DataFrame)
    assert isinstance(test_s3.get_df_voxel(spacing=np.asarray([0,1,2]),origin=np.asarray([0,1,2])),pd.DataFrame)
...
def test_get_graph():
    ...
    df_voxel = test_swc.get_df_voxel(spacing=np.asarray([1,2,3]),origin=np.asarray([1,2,3]))
    df_voxel_s3 = test_s3.get_df_voxel(spacing=np.asarray([1,2,3]),origin=np.asarray([1,2,3]))
    #swc
    ...
    #s3
    G_s3 = test_s3.get_graph(spacing=np.asarray([1,2,3]),origin=np.asarray([1,2,3]))
...
def test_generate_df_subset():
    ...
    with pytest.raises(TypeError):
        ...
        test_swc.generate_df_subset(vox_in_img_list,subneuron_start='asdf',subneuron_end=3)
    
    #test 'subneuron_end' arg must either be NoneType or int
    with pytest.raises(TypeError):
        vox_in_img_list = [[72,53,128],[69,60,122],[69,63,115],[74,63,108],[155,162,57]]
        test_swc.generate_df_subset(vox_in_img_list,subneuron_start=3,subneuron_end='asdf')
    ...
    assert isinstance(test_swc.generate_df_subset(vox_in_img_list,subneuron_start=0,subneuron_end=3), pd.DataFrame)
...
def test_get_sub_neuron():
    ...
    #test if 'spacing' is type numpy.ndarray, it must be shape (3,1)
    with pytest.raises(ValueError):
        test_swc.get_sub_neuron(bounding_box=[[1,2,4],[1,2,3]],spacing=np.asarray([1,2]))
    
    #test 'origin' arg must either be NoneType or numpy.ndarray
    with pytest.raises(TypeError):
        test_swc.get_sub_neuron(bounding_box=[[1,2,4],[1,2,3]],spacing=np.asarray([1,2,3]),origin=1)
    
    #test if 'origin' is type numpy.ndarray, it must be shape (3,1)
    with pytest.raises(ValueError):
        test_swc.get_sub_neuron(bounding_box=[[1,2,4],[1,2,3]],spacing=np.asarray([1,2,3]),origin=np.asarray([1,2]))
